...
class ChooseVendorModalForm extends React.Component {
    ...
    handleChangePage = (event, page) => {
        const { getDataTable, changeVendorsPagination, paginationData} = this.props
    ...
    handleChangeRowsPerPage = event => {
        const { getDataTable,
            ...
            changeVendorsPagination({ ...paginationData, elementsPerPage: event.target.value });
        getDataTable();
    };
   ...
   sortData = (property, order) => {
    ...
    render(){
        const { data, sortingData, paginationData, getDataTable, getFilteredDataTable, technicalServs} = this.props
        ...
        const actions = {handleRequestSort: this.handleRequestSort , handleChangePage: this.handleChangePage, handleChangeRowsPerPage: this.handleChangePage}
        return(
            ...
            >
             <DialogTitle id="form-dialog-choose">
              <div style={{ display: 'flex', justifyContent: 'space-between'}}>
                ...
                <IconButton style={{ float: 'right' }} color="secondary" onClick={this.cancel} >
                    <Clear/>
                 </IconButton>
              </div> 
            </DialogTitle>
            <DialogContent>   
                <Filters technicalServs={technicalServs} applyFilter={getFilteredDataTable} reset={getDataTable}/>
                <Paper>
                    <VendorsTable data={data} sortingData={sortingData} paginationData={paginationData} emptyRows={emptyRows} actions={actions} add={this.add}/>
               </Paper>             
          </DialogContent>         
         </Dialog>    
        )
...
export default connect(mapStateToProps, mapDispatchToProps)(ChooseVendorModalForm);