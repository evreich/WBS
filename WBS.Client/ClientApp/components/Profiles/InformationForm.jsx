            ...
            >
             ...
             <DialogTitle style={{backgroundColor: '#296E50'}} id="form-dialog-title">
              <div style={{ display: 'flex', justifyContent: 'space-between'}}>
                <div style={{ marginTop: 5 , color: '#fafafa',}}> Пользователи- {elem.fio}</div>
              </div>
            </DialogTitle>
            <DialogContent>
                ...
                <table maxWidth="650" border="0" cellpadding="5" cellspacing="0">
                    ...
                    <tr style={{height: '55px'}} >
                        <td>ФИО:</td> 
                        <td><div style={{ fontWeight: 'bolder'}}>{elem.fio}</div></td>
                        <td> {!elem.deletionMark && <ButtonDelete style={{marginLeft: '25%', marginRight: '5%'}} onClick={this.handleDeleteButtonClick} size={'small'} />}
                        <ButtonUpdate onClick={this.handleUpdateButtonClick} size={'small'} /></td>
                    </tr>
       
                    <tr style={{height: '55px'}}>
                        <td>Должность:</td>
                        <td><div style={{ fontWeight: 'bolder'}}>{elem.jobPosition}</div></td>
                    </tr>
                    <tr style={{height: '55px'}}>
                        <td>Подразделение:</td>
                        <td><div style={{ fontWeight: 'bolder'}}>{elem.department}</div></td>
                    </tr>
                    <tr style={{height: '55px'}}>
                        <td>Помечено на удаление:</td>
                        <td><div style={{ fontWeight: 'bolder'}}>{elem.deletionMark?"да":"нет"}</div></td>
                    </tr>
